...
class MazeRenderer:
    """ASCII maze renderer with path 'draw-in' animation when a path (ordered) is provided.
    If path is None -> no path shown. If path is a set -> static full-path. If path is an ordered
    iterable (list/tuple) -> animate draw-in.
    """
    ...
    def _build_static_lines(self, reveal_path: Optional[Set[Tuple[int,int]]] = None) -> List[str]:
    ...
    def _required_size(self) -> Tuple[int,int]:
        ...
        width = len(self.maze[0])
        cols = 0
        # top border: each cell contributes 4 chars ("+---" or "+  ") plus final "+"
        cols = 1 + width * 4
    ...
    def render(self) -> str:
        """Render maze. Print/animate frames to stdout; return final maze string."""
        if not self.maze or not self.maze[0]:
            out = "Empty maze"
        ...
        if term.columns < req_cols or term.lines < req_rows:
            msg = (f"Terminal too small: need {req_cols}×{req_rows}, "
        ...
        final = "\n".join(self._build_static_lines(reveal_path=set(seq)))
        # leave final frame on screen (already printed), but ensure final is returned
        return final
